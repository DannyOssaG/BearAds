...
function getPrimaryMembership(userId) {
  return getUserMemberships(userId).sort((a, b) => {
    const order = { owner: 0, admin: 1, billing: 2, developer: 3, member_paid: 4, member_trial: 5, member: 6, manager: 7, viewer: 8 };
...
function rolePermissions(role = 'member_trial') {
  ...
  return {
    canView: true,
    canEdit: ['owner','admin','developer','billing','member_paid','member_trial'].includes(r),
